...
import { Card, StatCard, Badge, Button, ProgressBar, Spinner, EmptyState } from '@/components/ui'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (paymentSuccess) {
      ...
      setTimeout(() => window.history.replaceState({}, '', '/dashboard'), 3000)
    }
  }, [searchParams])
      ...
      <div className="flex min-h-[calc(100vh-65px)]">
        ...
        <aside className="hidden w-56 shrink-0 border-r border-border bg-bg2 p-4 md:flex md:flex-col">
          ...
          <div className="mt-4 border-t border-border pt-4">
            <div className="text-xs uppercase tracking-widest text-text3 mb-2">Account</div>
          ...
          <div className="mt-auto rounded-xl border border-accent/25 bg-accent/8 p-4">
            ...
            <div className="text-xs text-text3">
              {usage?.exports_this_month || 0}/{usage?.export_limit || 5} exports used
            </div>
            <ProgressBar value={usage?.exports_this_month || 0} max={usage?.export_limit || 5} />
            {(sub?.plan === 'starter' || !sub?.plan) && (
              <Link href="/pricing">
                <Button size="sm" className="mt-3 w-full">Upgrade Now</Button>
              </Link>
            )}
          </div>
        </aside>
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            />
            <StatCard label="Storage Used"     value={`${storageUsedMB} MB`}           change={`${storagePct}% of ${storageLimitMB >= 1024 ? `${storageLimitMB/1024}GB` : `${storageLimitMB}MB`}`} changeType="neutral" />
              ...
              <div className="flex h-24 items-end gap-1.5">
                {[1,3,0,2,4,3,1].map((v, i) => (
                  <div
                    ...
                    style={{
                      height: `${(v/4)*100}%`,
                      background: i === 4 ? 'var(--accent)' : 'rgba(124,111,247,0.3)',
                      minHeight: v === 0 ? '4px' : undefined,
                    }}
                  />
                ))}
              </div>
